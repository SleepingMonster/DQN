...
from utils_drl import Agent  #导入了utils_drl的Agent类
...
class MyEnv(object):   
    ...
    #执行action并observe,得到相应reward并得到是否为最终状态
    ...
    def make_state(obs_queue: deque) -> TensorStack4:
        ...
        return torch.cat(list(obs_queue)[1:]).unsqueeze(0) #unsqueeze表示在第0维上增加一个维度
    ...
    def evaluate(
            ...
            obs_queue: deque,
            agent: Agent,
            num_episode: int = 3, #3步作为一个episode
            render: bool = False,
    ) -> Tuple[
        float,
        List[GymImg],
        ...
        for _ in range(self.get_eval_lives() * num_episode):
            ...
            while not done:
                ...
                action = agent.run(state)   #agent产生action
                obs, reward, done = self.step(action)  #环境执行action并观察得到相应reward
